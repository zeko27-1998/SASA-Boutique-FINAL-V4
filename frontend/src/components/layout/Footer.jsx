...
export default function Footer() {
  ...
  const helpLinks = [
    { label: t('footer_track'),       to: '/track-order' },
    { label: t('footer_returns'),     to: '#' },
    { label: t('footer_size_guide'),  to: '#' },
    { label: t('footer_faq'),         to: '#' },
    { label: t('footer_contact_us'),  to: '#' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
          ...
          <div>
            ...
            <ul className="space-y-3 mb-5">
              ...
              <li className={`flex items-center gap-2.5 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <span className="text-sm text-gray-600 font-semibold" dir="ltr">+964 770 000 0000</span>
              </li>
              <li className={`flex items-center gap-2.5 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <div className="w-7 h-7 rounded-full bg-pink-50 flex items-center justify-center flex-shrink-0">
                  <Mail className="w-3.5 h-3.5 text-[#C9A84C]"/>
                </div>
                <span className="text-sm text-gray-600 font-semibold" dir="ltr">hello@sasaboutique.iq</span>
              </li>
            </ul>
        ...
        <div className="border-t border-pink-100 pt-6
          ...
          <p className="text-xs text-gray-400 font-semibold flex items-center gap-1">
            ...
            {isRTL ? 'في العراق' : 'in Iraq'}
          </p>

          <div className="flex gap-4">
            {[t('footer_privacy'), t('footer_terms'), t('footer_cookies')].map(tx => (
              <a key={tx} href="#"
                className="text-xs text-gray-400 font-semibold hover:text-pink-500 transition-colors">
                {tx}
              </a>
            ))}
          </div>
